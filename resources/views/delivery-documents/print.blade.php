    ...
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="header-center">
                ...
                <h2 style="margin-top: -10px;">Delivery Document</h2>
                <h2 style="font-size: 20px; margin-top: 2px;">سند تسليم</h2>
            </div>
        ...
        <div class="date-line">
            <div class="date-item arabic-date">
                ...
                <input type="text" class="date-input"
                    value="{{ \Carbon\Carbon::parse($deliveryDocument->date_and_time)->format('Y/m/d') }}" readonly>
            </div>
            <div class="date-item center-item">
                <span
                    style="font-weight: 600; font-size: 13px;">{{ $deliveryDocument->purchase_order_no ? 'DEL-' . $deliveryDocument->purchase_order_no : ($deliveryDocument->document_number ?? 'DEL-' . $deliveryDocument->id) }}</span>
            </div>
        ...
        <div class="table-container compact-table">
            <table class="transporter-table">
                ...
                <tr>
                    ...
                    <td>{{ $deliveryDocument->driver_id_number ?? ($deliveryDocument->transporter->id_number ?? '') }}
                    </td>
                    <td>{{ $deliveryDocument->driver_name ?? ($deliveryDocument->transporter->driver_name ?? $deliveryDocument->transporter->name ?? '') }}
                    </td>
                    <td>{{ $deliveryDocument->car_no ?? ($deliveryDocument->transporter->car_no ?? '') }}</td>
                    <td>{{ $deliveryDocument->transporter->document_no ?? '' }}</td>
                    <td>{{ $deliveryDocument->transportCompany->name ?? ($deliveryDocument->transporter->name ?? '') }}
                    </td>
                </tr>
            </table>
        </div>
