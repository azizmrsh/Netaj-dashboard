...
<head>
    ...
    <title>سند استلام - {{ $receiptDocument->document_number ?? 'غير محدد' }}</title>
    ...
    <style>
        ...
        * {
        ...
        .transporter-table th:nth-child(1) {
            width: 12%;
        }

        .transporter-table th:nth-child(2) {
            width: 12%;
        }

        .transporter-table th:nth-child(3) {
            width: 18%;
        }

        .transporter-table th:nth-child(4) {
            width: 14%;
        }

        .transporter-table th:nth-child(5) {
            width: 14%;
        }

        .transporter-table th:nth-child(6) {
            width: 30%;
        }
    ...
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="header-center">
                ...
                <h2 style="margin-top: -10px;">Receipt Document</h2>
                <h2 style="font-size: 20px; margin-top: 2px;">سند استلام</h2>
            </div>
        ...
        <div class="date-line">
            <div class="date-item arabic-date">
                ...
                <input type="text" class="date-input"
                    value="{{ \Carbon\Carbon::parse($receiptDocument->date_and_time)->format('Y/m/d') }}" readonly>
            </div>
            <div class="date-item center-item">
                <span
                    style="font-weight: 600; font-size: 13px;">{{ $receiptDocument->purchase_invoice_no ? 'REC-' . $receiptDocument->purchase_invoice_no : ($receiptDocument->document_number ?? 'REC-' . $receiptDocument->id) }}</span>
            </div>
        ...
        <div class="intro-text">
            <div class="bilingual">
                <div class="arabic">تم استلام المواد من المورد حسب المعلومات التالية:</div>
                <div class="english">The following materials were received from the supplier as detailed below:</div>
            </div>
        </div>
        ...
        <div class="section-title compact-section">
            <div class="bilingual">
                <div class="arabic">معلومات المورد</div>
                <div class="english">Supplier Information</div>
            </div>
        </div>
                ...
                <tr>
                    <th>
                        <div class="bilingual">
                            ...
                            <div class="english">Phone No</div>
                        </div>
                    </th>
                    <th>
                        <div class="bilingual">
                            <div class="arabic">مصدر المواد</div>
                            <div class="english">Material Source</div>
                        </div>
                    </th>
                    <th>
                        <div class="bilingual">
                    ...
                    <th>
                        <div class="bilingual">
                            <div class="arabic">اسم المورد</div>
                            <div class="english">Supplier Name</div>
                        </div>
                    </th>
                </tr>
                <tr>
                    <td>{{ $receiptDocument->supplier->phone ?? '' }}</td>
                    <td>{{ $receiptDocument->material_source ?? '' }}</td>
                    <td>{{ $receiptDocument->purchase_invoice_no ?? '' }}</td>
                    <td>{{ $receiptDocument->supplier->name ?? '' }}</td>
                </tr>
            </table>
        </div>
        ...
        <div class="table-container compact-table">
            <table class="transporter-table">
                ...
                <tr>
                    <td>{{ $receiptDocument->driver_phone ?? ($receiptDocument->transporter->phone ?? '') }}</td>
                    <td>{{ $receiptDocument->driver_id_number ?? ($receiptDocument->transporter->id_number ?? '') }}
                    </td>
                    <td>{{ $receiptDocument->driver_name ?? ($receiptDocument->transporter->driver_name ?? $receiptDocument->transporter->name ?? '') }}
                    </td>
                    <td>{{ $receiptDocument->car_no ?? ($receiptDocument->transporter->car_no ?? '') }}</td>
                    <td>{{ $receiptDocument->transporter->document_no ?? '' }}</td>
                    <td>{{ $receiptDocument->transportCompany->name ?? ($receiptDocument->transporter->name ?? '') }}
                    </td>
                </tr>
            </table>
        </div>
        ...
        <div class="table-container compact-table">
            <table class="materials-table">
                ...
                <tbody>
                    @foreach($receiptDocument->receiptDocumentProducts as $index => $item)
                        <tr>
                    ...
                    @endforeach
                    @for($i = count($receiptDocument->receiptDocumentProducts); $i < 5; $i++)
                        <tr>
                            <td></td>
                            <td></td>
                            <td></td>
        ...
        <div class="table-container compact-table">
            <table class="signature-table">
                <tr>
                    ...
                    <th>
                        <div class="bilingual">
                            <div class="arabic">اسم وتوقيع مسؤول المشتريات</div>
                            <div class="english">Purchasing Officer Name & Signature</div>
                        </div>
                    </th>
                    <th>
                        <div class="bilingual">
                ...
                <tr>
                    <td class="name-field">{{ $receiptDocument->accountant_name ?? '' }}</td>
                    <td class="name-field">{{ $receiptDocument->warehouse_officer_name ?? '' }}</td>
                    <td class="name-field">{{ $receiptDocument->purchasing_officer_name ?? '' }}</td>
                    <td class="name-field">{{ $receiptDocument->recipient_name ?? '' }}</td>
                </tr>
                <tr>
                    <td></td>
                    <td></td>
        ...
        <div class="footer">
            <div class="bilingual">
                <div class="arabic">سند استلام</div>
                <div class="english">Receipt Document</div>
            </div>
        </div>
    </div>
</body>
